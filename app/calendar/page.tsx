    ...
    <div className="max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={prevMonth} className="px-3 py-1.5 border border-gray-300 rounded-md text-sm hover:bg-gray-50">‹ 이전</button>
          <button
          ...
          <button onClick={nextMonth} className="px-3 py-1.5 border border-gray-300 rounded-md text-sm hover:bg-gray-50">다음 ›</button>
        </div>
      </div>

      {/* 범례 */}
      <div className="flex gap-3 mb-4 text-xs">
        <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-red-400 inline-block" />기일</span>
        <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-blue-400 inline-block" />상담</span>
        <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-orange-400 inline-block" />상담예정</span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        ...
        <div className="grid grid-cols-7">
          {Array.from({ length: firstDay }).map((_, i) => (
            <div key={`empty-${i}`} className="border-b border-r border-gray-100 min-h-16 p-1" />
          ))}
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
            ...
            return (
              <div
                ...
                className={`border-b border-r border-gray-100 min-h-16 p-1 cursor-pointer transition-colors ${isSelected ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
              >
          ...
          {selectedEvents.length === 0 ? (
            ...
            <ul className="space-y-2">
              {selectedEvents.map((ev, i) => (
                <li key={i}>
                  <Link href={ev.href} className={`flex items-center gap-3 px-3 py-2 rounded-md border text-sm hover:opacity-80 transition-opacity ${TYPE_STYLE[ev.type]}`}>
